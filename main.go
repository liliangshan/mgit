...
MGIT_HOME=%s`, languages[index].Code, appExePath)
	...
	// 初始化数据库
	databaseA, err := db.InitDB(getDbFileName())
	if err != nil {
	...
	case "pull":
		...
		database, err = db.InitDB(getDbFileName())
		if err != nil {
	...
	case "pull-all":
		...
		for _, project := range projects {
			...
			err := executeInProjectDir(&project, func() error {
				// 使用 pull_branch 进行拉取
				pullBranch := project.PullBranch
				if pullBranch == "" {
					pullBranch = project.RemoteBranch
				}
				return git.GitPull(project.LocalBranch, pullBranch, false)
			})
	...
	case "new":
		...
		if _, err := os.Stat(newExePath); err == nil {
			log.Fatal(fmt.Errorf(i18n.T("error.app_already_exists"), newAppName))
		}
...
func initDbRepo(repoUrl string) error {
	...
	dbDir = filepath.Join(dbDir, ".mgit_db")
	...
	if _, err := os.Stat(dbDir); err == nil {
		...
		if err := git.GitOperation(true, "git", "rev-parse", "--git-dir"); err == nil {
			...
			if err == nil {
				...
				if existingRepo == repoUrl {
					...
					//判断数据库是否存在
					if _, err := os.Stat(filepath.Join(dbDir, "projects.db")); err == nil {
						...
						// 复制数据库文件
						if err := copyFile(filepath.Join(appExePath, "projects.db"), filepath.Join(dbDir, "projects.db")); err != nil {
					...
					if _, err := os.Stat(filepath.Join(dbDir, "projects.db")); err == nil {
						...
						if _, err := os.Stat(filepath.Join(appExePath, "projects.db")); err == nil {
							// 删除数据库文件
							if err := os.Remove(filepath.Join(appExePath, "projects.db")); err != nil {
								return fmt.Errorf(i18n.T("msg.db_file_delete_failed"), err)
							}
						}

						if err := copyFile(filepath.Join(dbDir, "projects.db"), filepath.Join(appExePath, "projects.db")); err != nil {
							return fmt.Errorf(i18n.T("msg.db_file_copy_failed"), err)
						}
					}
					// 复制数据库文件
		...
		if _, err := os.Stat(filepath.Join(dbDir, "projects.db")); err == nil {
			// 删除数据库文件
			if err := os.Remove(filepath.Join(dbDir, "projects.db")); err != nil {
				return fmt.Errorf(i18n.T("msg.db_file_delete_failed"), err)
			}
		}
		// 复制数据库文件
...
func syncDatabase(operation string) error {
	...
	dbDir := filepath.Join(os.Getenv("APP_PATH"), ".mgit_db")
	currentDir, err := os.Getwd()
	if err != nil {
		return fmt.Errorf(i18n.T("msg.dir_get_failed"), err)
	}
